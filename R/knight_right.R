...
#' This movement is derived from the [knight()] piece.
#' 
#' 
...
#' If `degree = 2`, 4 neighbors will be identified (except if 
#' `directed = TRUE`). The knight right must be used only if `degree > 1`.
#'  
#' If `directed = TRUE` and `reverse = FALSE`, only 2 nodes will be
#' considered as neighbors (remove the nodes at the bottom of the focus node).
#' 
#' If `directed = TRUE` and `reverse = TRUE`, only 2 nodes will be
