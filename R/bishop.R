...
#' two diagonal in both orientations (default behavior). 
#' 
#' 
...
#' 
#' 
#' The detection of neighbors using the bishop method can only work with 
...
bishop <- function(nodes, focus, degree = 1, directed = FALSE, reverse = FALSE, 
  ...
  if (length(unique(nodes$"quadrat")) == 1) {
    stop("The bishop movement is not designed to work through quadrats only. ",
         "Please use fool() instead.", call. = FALSE)
  }
  
  if (length(unique(nodes$"transect")) == 1) {
    stop("The bishop movement is not designed to work through transects only. ",
