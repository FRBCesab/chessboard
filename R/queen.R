...
#' If `degree = 2`, 16 neighbors will be identified (except if 
#' `directed = TRUE`).
#'  
#' If `directed = TRUE` and `reverse = FALSE`, only 10 nodes will be
#' considered as neighbors (remove the nodes at the bottom of the focus node).
#' 
#' If `directed = TRUE` and `reverse = TRUE`, only 10 nodes will be
...
queen <- function(nodes, focus, degree = 1, directed = FALSE, reverse = FALSE, 
  ...
  if (length(unique(nodes$"quadrat")) == 1) {
    stop("The queen movement is not designed to work through quadrats only. ",
         "Please use fool() instead.", call. = FALSE)
  }
  
  if (length(unique(nodes$"transect")) == 1) {
    stop("The queen movement is not designed to work through transects only. ",
