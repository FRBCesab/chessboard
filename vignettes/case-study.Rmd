...
The package `chessboard` implements the following rules to detect neighbors and 
create edges:
...
`chessboard` can also deal with one-dimensional samplings (either **transects-only** or 
...
the package only). This step is automatic and is performed when calling the function `create_nodes_labels()`.


\


For instance, if the sampling has been conducted only 
through transects, a fictitious quadrat will be created (Fig. 2).
...
fictitious transect will be created (Fig. 3).
...
- from bottom to top (along transects) for quadrats
- from left to right (along quadrats) for transects
...
The function `create_nodes_labels()` is used to create nodes labels of the form: `1-2`, where `1` is the identifier of 
...
| Function name  | 2D network | Transect only | Quadrat only | Description                                              |
| :------------- | :--------: | :-----------: | :----------: | :------------------------------------------------------- |
...
| `knight()`^\*^ |     ✅     |      ❌       |      ❌      | Difference between the `wizard()` and the `queen()`      |
| `queen()`      |     ✅     |      ❌       |      ❌      | Finds neighbors horizontally, vertically, and diagonally |
| `wizard()`     |     ✅     |      ❌       |      ❌      | Finds neighbors in all directions                        |


^\*^ These two functions have two variants: `bishop_left()`, `bishop_right()` and `knight_left()`, `knight_right()` for creating complex moves.
