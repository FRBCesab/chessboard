...
nodes <- create_nodes_labels(transects = sites$"transect", 
                             quadrats  = sites$"quadrat")
...
The aim of the package `bridge` is to make easier the creation of connectivity 
...
and/or vertically and/or diagonally? The package `bridge` implements all possible
...
It's important to note that, even the package `bridge` handles spatial networks, 
...
`bridge`. All are derived from the chess game and allow user to define 
complex movements.
