...
Instead, it will identify neighbors only based on **node labels**. In other 
words, the order of node labels will give the dimension and the direction of 
the network.
...
When using the package `chessboard`, the **first step** is to create **node labels**
...
Node labels are of the form: `1-2`, where 
...
The function `get_node_list()` can be used to extract the node list.

```{r 'get-node-labels'}
## Get node labels ----
get_node_list(adour_sites)
```
...
## Create edge list ----
adour_edges_pawn <- create_edge_list(nodes    = adour_sites, 
                                     method   = "pawn", 
                                     degree   = 1, 
                                     directed = TRUE,
                                     reverse  = FALSE,
...
adour_edges_bishop <- create_edge_list(nodes    = adour_sites, 
                                       method   = "bishop", 
                                       degree   = 1, 
                                       directed = TRUE,
                                       reverse  = FALSE,
