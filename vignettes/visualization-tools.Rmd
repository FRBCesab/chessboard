...
(see the [Case study](https://frbcesab.github.io/chessboard/articles/case-study.html) 
...
On this chessboard, we can locate one specific node using the function `geom_piece()`.
...
We can call the function `geom_piece()` several times to emphase various nodes.
...
Now let's call the function `bishop()` to create a second edges list.
...
As for the function `geom_piece()`, we can call the functions `geom_edges()` 
...
For the rest of the vignette let's create the edges list using the queen move.
...
From this edges list, we can build the associated connectivity matrix with the
...
To convert our edges list into a spatial object, we can use the function `edges_to_sf()`.
