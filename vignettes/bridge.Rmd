...
1) the list of nodes and 
2) the list of edges (links) between nodes.
...
The function `adjacency_matrix()` only requires the edges list as argument.
...
The package `bridge` also provides some useful functions to compute spatial 
weights required by MEM and AEM methods.
...
The simplest way to create spatial weights is to compute distances between pairs
of nodes. Two methods are implemented in `bridge`: the Euclidean distance and 
the distance according to the shape of the river.
...
The Euclidean distance, a straightforward metric to compute distances between pairs
of nodes, is implemented in the function `distance_euclidean()`. This function only requires
the coordinates of sites (object `sf`) and will compute distances between each
...
The function `distance_along()` implements of more complex way to compute distances
...
### Edges weighting matrix


An edges weighting matrix is a matrix of dimensions `n x n`, where `n` is the 
number of nodes (sites). This matrix contains the weight values of edges and is
a simple transposition of the `data.frame` created either by `distance_euclidean()`
or by `distance_along()`.


\


Let's create this edges weighting matrix for our network.


```{r 'edges-weighting-matrix'}
# Edges weighting matrix ----
adour_edges_weights <- edges_weight_matrix(adour_dists)
round(adour_edges_weights)
```

Note that this matrix must have the same dimensions as the connectivity matrix.
