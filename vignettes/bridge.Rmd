...
between pairs of nodes. Different network objects can be produced by `bridge`:
...
The package `bridge` is designed to work with **two-dimensional network** (i.e. 
sampling on a regular grid). The network can be spatial or not, and can be 
directed or undirected.

`bridge` provides an example of a survey realized along the French river
_L'Adour_ (Fig. 1).
...
## Import sites data ----
path_to_file <- system.file("extdata", "adour_survey_sampling.csv", package = "bridge")
...
**N.B.** The package `bridge` can also deal with one-dimensional sampling 
...
It's **important** to note that, even the package `bridge` can handle spatial 
...
When using the package `bridge`, the **first step** is to create **nodes labels**
