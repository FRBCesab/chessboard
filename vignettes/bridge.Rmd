...
weight matrix/vector, nodes by edges matrix, etc. These objects can be used later in 
...
This vignette shows only the usage of the package `bridge` with **directed spatial network**.
...
**Note:** this object is useless to worl with AEM, but can useful to check the edges
list.
...
influencing a site, directly or indirectly.
...
of the function `aem.build.binary()` of the package `adespatial`. This function 
...
## Edges weights (Euclidean distance)
...
## Edges weights (distance along river)
...
The second object (optional) requires by AEM is the edges weights vector. It is
a simple numeric vector giving the weight of each edge. Here we are going to use
the Euclidean distance an edge weight.
...
# Edges weights vector ----
adour_weights <- edges_weights_vector(adour_dists)
adour_weights
```
...
Now we can run an AEM as follow: 
`adespatial::aem(adour_edges_mat, weight = adour_weights)`
